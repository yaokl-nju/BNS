...
stopping_args = dict(
        stop_varnames=[StopVariable.LOSS, StopVariable.ACCURACY],
...
class EarlyStopping:
    def __init__(
        ...
        self.remembered_vals = [np.inf, -np.inf]
    ...
    def check(self, values: List[np.floating], epoch: int) -> bool:
        ...
        if any(checks):
            # ===  记录最好的评价指标
            self.best_vals = np.choose(checks, [self.best_vals, values[:2]])
            self.patience = self.max_patience
            ...
            if self.remember is Best.ALL:
                ...
                if all(comp_remembered):
                    ...
                    self.remembered_vals = copy.copy(values[:2])
